...
#define CLOSURE_DECLARE(name, rettype, args) rettype (**name) args

#define CLOSURE_TYPE(name, ...) ???

//CLOSURE_DEFINE(cap_is_divisible_t, (
#define CLOSURE_TYPEDEF(name, rettype, argstypes) typedef rettype  
...
struct cap_is_divisible cap_is_divisible(int val) {
	return (struct cap_is_divisible){ .val = val, .lambda = cap_is_divisible_cb };
}
